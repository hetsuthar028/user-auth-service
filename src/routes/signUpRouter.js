...
// @Route - SignUp
signUpRouter.post("/api/user/signup", (req, res) => {
  ...
  dbObj.beginTransaction((err) => {
    ...
    dbObj.query(userExistsCheckQuery, (err, results) => {
      ...
      if (results.length != 0) {
        return res.status(400).json({ warning: "User already exists" });
      }
      ...
      newPasswd.then((password) => {
        ...
        dbObj.query(userAddQuery, (err, results) => {
          ...
          if (userType == "developer") {
            ...
            dbObj.query(developerAddQuery, (err, results) => {
              if (err) {
                console.log("Can't Add Developer Data", err);
                return dbObj.rollback();
              }
              dbObj.commit();
            ...
            dbObj.query(organizationAddQuery, (err, results) => {
              if (err) {
                console.log("Can't Add Organization Data", err);
                return dbObj.rollback();
              }
              dbObj.commit();
              return res.status(200).json({message: "success"})
